...
# V2 Changes:
#
# Variable $secureCredentials was removed from this version. Authentication is done with logged on account.
# Changes was done to be used with Jenkins OpenSSH client
#
#################################################################################################################
# Variables
...
$DefaultmaxPasswordAge = (Get-ADFineGrainedPasswordPolicy -Identity $adFineGrainedPasswordPolicy -Properties * -Server $DomainName.Domain | Select MaxPasswordAge).MaxPasswordAge
...
foreach ($user in $users) {
    ...
    $PasswordPol = (Get-AduserResultantPasswordPolicy $user -Server $DomainName.Domain)
